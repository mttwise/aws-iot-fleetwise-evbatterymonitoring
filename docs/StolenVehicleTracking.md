...
**[Stolen Vehicle Tracking on AWS](https://aws.amazon.com/solutions/implementations/Stolen-Vehicle-Tracking-solution-on-aws/)** | **[🚧 Feature request](https://github.com/aws-solutions/Stolen-Vehicle-Tracking-solution-on-aws/issues/new?assignees=&labels=enhancement&template=feature_request.md&title=)** | **[🐛 Bug Report](https://github.com/aws-solutions/Stolen-Vehicle-Tracking-solution-on-aws/issues/new?assignees=&labels=bug&template=bug_report.md&title=)** | **[❓ General Question](https://github.com/aws-solutions/Stolen-Vehicle-Tracking-solution-on-aws/issues/new?assignees=&labels=question&template=general_question.md&title=)**
...
## Table of Contents
- [Stolen Vehicle Tacking](#Stolen-Vehicle-Tracking-solution-on-aws)
  ...
  - [Architecture Diagrams](#architecture-diagrams)
    ...
    - [SVL Backstage Architecture Diagram](#SVL-backstage-architecture-diagram)
  ...
  - [Deploy](#deploy)
    ...
    - [Deploy SVL Modules via Backstage](#deploy-SVL-modules-via-backstage)
      - [SVL Module Deployment Order](#SVL-module-deployment-order)
...
[Stolen Vehicle Tacking Guidance](https://aws.amazon.com/solutions/implementations/Stolen-Vehicle-Tracking-solution-on-aws/)
solution page.

## Architecture Diagrams

### Solution Architecture Diagram
![Solution Architecture Diagram](./architecture/diagrams/SVL-all-modules-architecture-diagram.svg)

### Module Deployment Sequence Diagram
![Module Deployment Sequence Diagram](./sequence/SVL-module-deployment-sequence-diagram.svg)

### Environment

For reference, there is a proton environment setup with further
details in its [README](./templates/environments/SVL_environment/v1/infrastructure/README.md).


#### Example Module Deployment via Automotive Cloud Developer Portal

The following instructions detail how to deploy the SVL Vehicle Simulator Module.
The same steps can be applied to other modules as well by replacing the URLs and names.

1.	Navigate to the SVL Backstage URL in a web browser (ROUTE53_BASE_DOMAIN that was specified during deployment).
...
4. On Backstage, navigate to the `Create` page available from the `Catalog` menu in the side bar. Select the `CHOOSE` button on the `SVL Vehicle Simulator on AWS` card.
...
[implementation guide](https://docs.aws.amazon.com/solutions/latest/Stolen-Vehicle-Tracking-solution-on-aws/operational-metrics.html).
...
2. Delete SVL on AWS Modules in AWS Proton and CloudFormation that were deployed via Backstage
   1. In AWS Proton, Navigate to the `Services` view and delete any service attached to the
   `SVL-environment` Environment. Wait until all services successfully delete.

      > **NOTE:** You have to click the link into the service to be able to delete it via the `Actions` dropdown

   ![Delete SVL Module Proton Service](documentation/images/readme/delete-SVL-module-proton-service.png)
      ...
      In this case, go to `Account Settings` and configure a role.
         > **WARNING:** Proton's UI requires a GitHub Repository connection to configure roles.  To get around this, set the roles using a CLI command.
            ...
            --pipeline-codebuild-role-arn arn:aws:iam::11111111111:role/SVL-dev-SVLprotonenvironmentprotoncodebuildroleXXX-XXXXXX \
   ...
   1. After AWS Proton shows that all services have been deleted, verify in the
      CloudFormation console that all SVL on AWS modules have been deleted, and if not, delete them.
...
1. Delete AWS Proton Service Templates, Environment, and Environment Templates.
   ...
   1. Navigate to the `Service Templates` view in AWS Proton and delete any SVL on AWS service templates.
   ...
   2. Navigate to the `Environments` view and delete the `SVL_environment` Environment.
   ...
   3. Navigate to the `Environment Templates` view and delete the `SVL_environment` Environment Template.
   ...
   6. Verify that the `SVL-environment` stack was removed when tearing down AWS Proton, and if not, delete it.
